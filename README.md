...
3.3 이동,애니메이션
3.4 적이 가까히 오거나 a키 조작으로 공격가능한 범위에 있으면 공격을 한다
3.5 회피 스킬을 획득했을 경우 F키를 누르면 회피스킬을 사용할수있고 해당 스킬은 쿨타임을 가진다
3.6 q,w,e로 스킬을 사용할수있다 스킬 사용범위를 넘어가면 스킬은 사용이 취소가 된다
3.7. 스킬을 누르면 스킬의 범위,스킬 적용 범위가 나온다
